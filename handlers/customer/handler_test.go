...
func TestHandler_Create(t *testing.T) {
	...
	cases := []struct {
		...
		{"create new entity", bytes.NewReader([]byte(`{"id":1,"name":"Aryan","address":"","phone_no":0}`)), []byte(`{"id":1,"name":"Aryan","address":"","phone_no":0}`), http.StatusCreated},
		{"invalid parameter", bytes.NewReader([]byte(`{"NAME":"Ruchit"}`)), []byte(""), http.StatusBadRequest},
		{"missing parameter", bytes.NewReader([]byte(`{"name":"Ruchit"}`)), []byte(""), http.StatusBadRequest},
		...
		{"unable to read body", mockReader{}, []byte(""), http.StatusInternalServerError},
	}
...
func TestHandler_DeleteByID(t *testing.T) {
	...
	for i, tc := range cases {
		req := httptest.NewRequest(http.MethodDelete, "http://customer", http.NoBody)
		r := mux.SetURLVars(req, map[string]string{"id": tc.id})
		w := httptest.NewRecorder()
